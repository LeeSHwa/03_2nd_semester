...
public class JpaApiNativeSQLTest {
    ...
    @Test
    @Disabled   // H2 NOT SUPPORT SIMPLE WITH
    @SuppressWarnings("unchecked")
    void test_nativeQuery_simple_with_sql() {
        ...
        var sql =
            ...
            SELECT d.* FROM MDepartment d'
            """;
        Long sum = (Long) entityManager
    ...
    @Test
    @SuppressWarnings("unchecked")
    void test_nativeQuery_recusive_with_sql() {

        // H2 SUPPORTS RECURSIVE WITH QUERY
        var sql =
            """
    ...
    @Test
    @SuppressWarnings("unchecked")
    void test_nativeQuery_using_entity_mapping() {
    ...
    @Test
    @SuppressWarnings("unchecked")
    void test_nativeQuery_using_Hibernate6_TupleTransformer_lambda() {

        org.hibernate.Session session = entityManager.unwrap(org.hibernate.Session.class);

        var sql = "SELECT d.id, d.name FROM Department d";
        List<DepartmentValue> departments = session
            .createNativeQuery(sql, Object[].class)
            .setTupleTransformer((tuple, aliases) -> {
                    log.info("Transform tuple");
                    DepartmentValue d = new DepartmentValue();
                    d.setId((Long) tuple[0]);
                    d.setName((String) tuple[1]);
                    return d;
            })
            .getResultList();
